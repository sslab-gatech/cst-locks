...
xeon) echo "running tests on xeon"
    THE_LOCKS="TTAS MCS TICKET SPINLOCK COHORT CST"
    num_cores=80
    platform_def="-DXEON"
    freq=2130000000
    make="make"
...
;;
xeon120) echo "running tests on xeon"
    THE_LOCKS="HCLH TTAS ARRAY MCS TICKET HTICKET MUTEX SPINLOCK CLH COHORT CST"
    num_cores=120
    platform_def="-DXEON120"
    freq=2130000000
    make="make"
    platform=xeon
    prog_prefix="numactl --physcpubind=1 ../"
...
do
cd ..; LOCK_VERSION=-DUSE_${prefix}_LOCKS PRIMITIVE=-DTEST_CAS OPTIMIZE=${optimize} PLATFORM=${platform_def} ${make} clean all -j; cd scripts;
