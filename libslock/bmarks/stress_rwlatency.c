...
#define DEFAULT_NUM_RDTHREADS 0
#define DEFAULT_NUM_WRTHREADS DEFAULT_NUM_THREADS
...
void *wrtest(void *data)
{
    ...
    while (stop == 0) {
        ...
        acquire_write(&local_d[lock_to_acq],&the_locks[lock_to_acq]);
        if (acq_duration > 0)
        {
            cpause(acq_duration);
        }
...
#endif
        release_write(&local_d[lock_to_acq],&the_locks[lock_to_acq]);
        if (acq_delay>0) {
#ifdef __tile__
            MEM_BARRIER;
#endif
...
void *rdtest(void *data)
{
    ...
    while (stop == 0) {
        ...
        acquire_read(&local_d[lock_to_acq],&the_locks[lock_to_acq]);
        if (acq_duration > 0)
        {
            cpause(acq_duration);
        }
        uint32_t i;
	volatile int value[cl_access];
#ifndef NO_DELAYS
        for (i = 0; i < cl_access; i++)
        {
                value[i] = protected_data[i + protected_offsets[lock_to_acq]].the_data[0];
		(void)value[i];
		COMPILER_BARRIER;
        }
#endif
        release_read(&local_d[lock_to_acq],&the_locks[lock_to_acq]);
        if (acq_delay>0) {
#ifdef __tile__
            MEM_BARRIER;
#endif
...
int main(int argc, char **argv)
{
	...
	{"wthreads", 		      required_argument, NULL, 't'},
        {NULL, 0, NULL, 0}
    };
    ...
    num_rdthreads = DEFAULT_NUM_RDTHREADS;
    num_wrthreads = DEFAULT_NUM_WRTHREADS;

    sigset_t block_set;

    while(1) {
        i = 0;
        c = getopt_long(argc, argv, "hl:d:n:w:a:p:c:r:t:", long_options, &i);
        ...
        switch(c) {
            ...
            case 'c':
                ...
                break;
	    case 'r':
		num_rdthreads = atoi(optarg);
		break;
	    case 't':
		num_wrthreads = atoi(optarg);
		break;
            case '?':
    ...
    assert(acq_delay >= 0);
    assert(cl_access >= 0);
    assert(num_rdthreads >= 0);
    assert(num_wrthreads >= 0);
    assert(num_threads == num_wrthreads + num_rdthreads);
	...
	if (i < num_rdthreads) {
		if (pthread_create(&threads[i], &attr, rdtest, (void *)(&data[i])) != 0) {
			fprintf(stderr, "Error creating thread\n");
			exit(1);
		}
	} else {
		if (pthread_create(&threads[i], &attr, wrtest, (void *)(&data[i])) != 0) {
			fprintf(stderr, "Error creating thread\n");
			exit(1);
		}
        }
