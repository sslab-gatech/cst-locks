...
int main(int argc, char **argv)
{
    ...
    double average_acquires = (double)acquires / num_threads;
#ifdef PRINT_OUTPUT
    printf("#average acquires	: %f\n", average_acquires);
#endif

    double variance = 0.0;
    for(i = 0; i < num_threads; i++) {
       variance += pow((data[i].num_acquires - average_acquires), 2);
    }
    variance = variance / num_threads;
#ifdef PRINT_OUTPUT
    printf("#variance	: %f\n", variance);
#endif
    ...
    int fdacq = 0;
    if((fdacq = open("/home/victoire/zpf/cst-locks/libslock/acquires_per_second_1000.txt", O_RDWR|O_CREAT|O_APPEND, S_IRUSR|S_IWUSR)) < 0){
        printf("can not open the file: %d\n", errno);
    }

    char buffer[128];
    sprintf(buffer, "#thread_count  : %02d        #acquires/s           : %f\n", num_threads, acquires * 1000.0 / duration);
    write(fdacq, buffer, strlen(buffer));
    close(fdacq);

    int fdstd = 0;
    if((fdstd = open("/home/victoire/zpf/cst-locks/libslock/stddev-average-ratio_1000.txt", O_RDWR|O_CREAT|O_APPEND, S_IRUSR|S_IWUSR)) < 0){
        printf("can not open the file: %d\n", errno);
    }

    sprintf(buffer, "#thread_count  : %02d        #stddeviation/average           : %f\n", num_threads, standard_deviation/average_acquires*100);
    write(fdstd, buffer, strlen(buffer));
    close(fdstd);
#ifdef PRINT_OUTPUT
    printf("#standard deviation/average		: %f\n", standard_deviation/average_acquires*100);
#endif
    int fdavg = 0;
    if((fdavg = open("/home/victoire/zpf/cst-locks/libslock/average_1000.txt", O_RDWR|O_CREAT|O_APPEND, S_IRUSR|S_IWUSR)) < 0){
        printf("can not open the file: %d\n", errno);
    }

    sprintf(buffer, "#thread_count  : %02d        #average           : %f\n", num_threads, average_acquires);
    write(fdavg, buffer, strlen(buffer));
    close(fdavg);
