...
- `USE_CST_LOCKS` - use ticket (local) + mcs (global) locks
- `USE_CSTMCS_LOCKS` - use mcs (local) + mcs (global) locks
- `USE_CSTMCSVAR_LOCKS` - use K42 (local) + mcs (global) locks
- `USE_CST_RW_LOCKS` - use CST read-write locks
- `USE_K42MCS_LOCKS` - use K42 locks
- `USE_COHORT_LOCKS` - Use cohort locks
...
- `XEON120` - 8 x 15-core Intel server
